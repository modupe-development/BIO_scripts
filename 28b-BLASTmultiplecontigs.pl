...
{   my $RUNname = "B.contig-".$job;                         # define the queue job name
    ...
    open(OUT,">z-BLASTtabularshell-$datetag.sh");
    print OUT $blastshell;
    print OUT "$RUNname\n\n";
    open (TORQUE, ">>$torquename");
    print OUT "/usr/local/blast-2.2.22/bin/megablast -d /usr/local/blast_db/nt -i $inseq -o $outfile -v 10 -m 8\n\n";
    print TORQUE "\n\n/usr/local/blast-2.2.22/bin/megablast -d /usr/local/blast_db/nt -i $inseq -o $outfile -v 10 -m 8\n";
    close(OUT);
    close(TORQUE);
    ...
    open(TORQUE, ">>$torquename");
    print `qsub z-BLASTtabularshell-$datetag.sh >>$torquename`;
    close(TORQUE);
